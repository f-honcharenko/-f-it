<template>
  <div class="hello">
    ...
    <select
      ...
      size="5"
      multiple="multiple"
      v-model="data.lessonGroup"
    >
      <option :value="keyN.name" v-for="keyN in options.groups" :key="keyN.id">
        {{ keyN.name }}
      </option>
    </select>
    <hr />
    ...
    <select
      name="select"
      :size="options.lessons.length"
      v-model="data.lessonName"
    >
...
export default {
  ...
  methods: {
    create() {
      ...
      axios
        .post("http://" + document.domain + ":5000/timetables/1/add", this.data)
        .then(
          (res) => {
            alert(res);
    ...
    testHTTP() {
      axios.post("http://" + document.domain + ":5000", { tes: "tes" }).then(
        (res) => {
          console.log(res);
        },
        (err) => {
    ...
    submit() {
      axios
        .put("http://" + document.domain + ":5000/options/group/addnew", {
          groupName: this.createGroup.name,
          groupSpec: this.createGroup.spec,
        })
        .then(
          () => {
            location.reload();
